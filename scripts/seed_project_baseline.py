...
## High concept
- {genre_name} baseline project focused on a small but high-quality first playable slice
...
- CI/build assumptions: local checks via `make ci-local`, optional container check via `make docker-verify`, GitHub workflows for validation and Docker smoke
...
## Build graph
- Source -> repo validation -> local evals -> engine-native smoke -> package -> artifact -> distribution
...
| Vertical Slice | Prove quality bar and retention promise | One polished slice with clear content, UX, and performance bar | Next milestone | Content throughput, readability |
