...
- The software pattern catalog below should help the agent choose the right ownership shape before it writes code: state projection, deterministic resolution, authority split, graph routing, scheduler queues, pooling/instancing, or diagnostic overlays.
...
Use this section when the question is not just "what genre is this?" but "what software pattern should own the genre promise?"

| Pattern | Use when | Common genres | Good example signals | Watch out |
| --- | --- | --- | --- | --- |
| State projection | Runtime truth must stay separate from UI, lore, or save screens | narrative adventure, life sim, grand strategy, colony-sim | UI reads projected state instead of mutating authority | UI quietly becoming the truth owner |
| Deterministic resolver | The player must understand why a turn, draft, or round resolved the way it did | deckbuilder-roguelike, auto-battler, tactical RPG, puzzle | logs explain the outcome and hidden state is minimized | hidden randomness or unreadable effect stacking |
| Authority split | Gameplay truth, presentation, and tooling need different owners | co-op survival, multiplayer, action roguelite, hero shooter | one system owns truth while others project or inspect it | ownership leaks across systems |
| Graph / route model | World memory, faction history, quest routing, or campaign state needs explicit edges | metroidvania, grand strategy, narrative adventure, world graph | nodes and edges are typed, queryable, and auditable | graph density outgrowing readability |
| Scheduler / job queue | Many agents or tasks need priority, fairness, or throughput control | colony-sim, factory-automation, life sim | queues, priorities, and delays are visible | starvation, opaque work queues, or hidden stalls |
| Spawn / pool / instance | Repeated objects, bullets, enemies, or props dominate runtime cost | survivorlike, action roguelite, city-builder, factory automation | reset rules are explicit and churn stays low | object-per-thing scale collapse |
| Perception / suspicion FSM | Detection, alertness, or stealth routing is the core tension | stealth, immersive sim, patrol games | visibility and suspicion states are readable | arbitrary detection or noisy state spikes |
| Overlay / diagnostic view | The player needs bottleneck diagnosis, not just simulation | city-builder, factory automation, colony-sim | overlays expose demand, flow, and bottleneck state | sim becoming unreadable without diagnostics |
| Dialogue / branch projection | Authored choices need visible runtime consequences | narrative adventure, RPG, quest-driven games | authored data and runtime branch state stay separate | branch explosion or hidden branch state |
| Board / lane resolution | Placement and round resolution are the core player decision | auto-battler, tactics, tower defense | board state and round logs make outcomes explainable | board opacity or unreadable synergy spikes |

## Genre to software pattern bundles

### Action roguelite
- Primary patterns: authority split, spawn/pool/instance, deterministic resolver, state projection.
- Use these when the game needs fast combat, readable pressure, and run variety without letting UI or save state own the combat model.

### Deckbuilder roguelike
- Primary patterns: deterministic resolver, state projection, graph / route model.
- Use these when the game needs explicit draw/discard state, route planning, and replayable outcome logs.

### Co-op survival
- Primary patterns: authority split, spawn/pool/instance, scheduler / job queue.
- Use these when the game needs session truth, shared scarcity, and co-op recovery without desync confusion.

### Auto-battler
- Primary patterns: board / lane resolution, deterministic resolver, economy projection.
- Use these when the game needs legible round resolution, shopping, and board placement that can be explained back to the player.

### Cozy sim and life sim
- Primary patterns: state projection, dialogue / branch projection, scheduler / job queue.
- Use these when the game needs routines, relationships, and long-session continuity without hidden state or UI friction.

### Extraction-lite
- Primary patterns: authority split, state projection, deterministic resolver.
- Use these when the game needs raid truth, stash truth, and extraction clarity instead of vague loss or loot rules.

### Grand strategy
- Primary patterns: graph / route model, state projection, scheduler / job queue.
- Use these when the game needs realm memory, diplomacy, event pacing, and campaign-scale save trust.

### Survivorlike
- Primary patterns: spawn / pool / instance, deterministic resolver, state projection.
- Use these when the game needs heavy density, frequent upgrades, and performance-safe repeated visuals.

### Narrative adventure
- Primary patterns: dialogue / branch projection, state projection, graph / route model.
- Use these when the game needs authored consequence and long-lived story state without branch explosion.

### Platformer, stealth, puzzle, and soulslike
- Primary patterns: perception / suspicion FSM, graph / route model, deterministic resolver.
- Use these when readability, recovery, and explicit rules are more important than systemic breadth.

## How to use the catalog
1. Name the dominant tension.
2. Pick the smallest software pattern that can own that tension.
3. Pick one example game that proves the pattern is real.
4. Write the first slice around that pattern instead of around the genre label.
5. Tell the agent which systems own truth, projection, and diagnostics before implementation.
