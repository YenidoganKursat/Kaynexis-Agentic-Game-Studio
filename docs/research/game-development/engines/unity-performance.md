...
- If the real bottleneck is GPU ownership or CPU-GPU communication rather than generic physics/query cost, pair this note with `docs/reference/gpu-guide.md` and `docs/examples/gpu-example.md` before tuning.

## Recommended order
- Measure one baseline first.
- Keep classic `GameObject` / Component / Prefab / `ScriptableObject` composition if it can still hit budget.
- Reduce layer collisions, trigger spam, and allocation churn before changing the whole architecture.
- Prefer `ObjectPool<T>` and `NonAlloc` queries for hot combat or sensing loops.
- Move to DOTS / Burst / Unity Physics only when the measured scale problem clearly outgrows the classic object model.
